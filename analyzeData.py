...
# Read files
datapath = "/uboone/data/users/sporzio/HighVoltageTask/ExtendedData/"
fDT = open(datapath+"DifferenceTracker.dat")
fPV = open(datapath+"PickOffVoltage.dat")
fCM = open(datapath+"CurrMon.dat")
fBL = open("Data/blips.dat","w")
fDT.readline()
fPV.readline()
fCM.readline()
...
while iDT < len(timeDT)-1:
    ...
    if bInteresting:
        ...
        c1.SaveAs("DataPlots/blip_"+str(int(timeDT[iDT]))+".png")
        outputline = "%i %s %s %s %s %f %f \n" %(timeDT[iDT],DTblipType,PVblipType,shortBaselineStatus,longBaselineStatus,peakob,fwhm)
        print outputline
